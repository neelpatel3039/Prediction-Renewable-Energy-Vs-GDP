...
# Function to train and test polynomial regression for each country
train_test_model <- function(country_data, country_name, degree = 2) {
  ...
  # Create formula for polynomial regression
  formula <- as.formula(paste("GDP ~ 
                               poly(Renew_Elec_Pct, ", degree, ", raw=TRUE)", sep=""))
...
for (country in countries) {
  ...
  result <- train_test_model(country_data, country, degree = 2)
