...
# Removing Continents, Regions, World and Martinique since it is not present in Hydro
...
# Standardise Precision Point to 3
filtered_master_dataset <- filtered_master_dataset %>%
...
# Read the GDP data
world_gdp <- read_csv("world-gdp.csv")

# Read the data
economy_classification <- read_csv("economy-classification.csv")
...
# Convert GDP to Million Dollars 
...
# ---------------------------------- (6) Remove all countries which NAs ------------------------ 

# Identify rows with at least one NA
rows_with_na <- master_data_with_gdp[rowSums(is.na(master_data_with_gdp)) > 0, ]

# Extract the unique countries with missing values
# countries_with_na <- unique(rows_with_na$Entity)

# master_data_with_gdp <- master_data_with_gdp[!master_data_with_gdp$Entity %in% countries_with_na, ]

# Remove rows with NA values (optional, depending on your data cleaning strategy)
#master_data_with_gdp <- master_data_with_gdp %>%
#  filter(!is.na(GDP))
...
write_csv(master_data_with_gdp, "master_data_with_gdp.csv")
